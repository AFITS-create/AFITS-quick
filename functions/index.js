...
function getGrokModel() {
  const model = safeAiText(process.env.GROK_MODEL || "", 80);
  // xAI retired/changes some older aliases. Keep old app env values from breaking chat.
  if (!model || model === "grok-3-mini") return "grok-4.3";
  return model;
}
